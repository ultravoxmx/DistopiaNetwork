...
public class PodcastApiClient
{
        ...
        KeyStore keys)
    {
        ...
        _baseUrl = _settings.ServerUrl.TrimEnd('/');
        _nonce = nonce;
        _signer = signer;
        _keys = keys;
    }
    ...
    public async Task<List<PodcastMetadata>> GetCatalogAsync(CancellationToken ct = default)
    {
        try
        {
            ...
            var result = await http.GetFromJsonAsync<List<PodcastMetadata>>($"{_baseUrl}/podcasts", ct);
            return result ?? [];
        }
        catch (Exception ex)
        {
            throw new HttpRequestException($"Catalog request failed to {_baseUrl}/podcasts. {BuildError(ex)}", ex);
        }
    }
    ...
    public async Task<OperationResponse> UpdateMetadataAsync(string podcastId, string title, string description, string? imageUrl, CancellationToken ct = default)
    {
        ...
        var http = CreateClient();
        var response = await http.PutAsJsonAsync($"{_baseUrl}/podcast/{podcastId}/metadata", request, ct);
    ...
    public async Task<OperationResponse> DeletePodcastAsync(string podcastId, string? reason, CancellationToken ct = default)
    {
        ...
        var http = CreateClient();
        var msg = new HttpRequestMessage(HttpMethod.Delete, $"{_baseUrl}/podcast/{podcastId}")
        {
    ...
    public async Task<PublishClientResult> PublishAsync(
        string filePath,
        string title,
        string description,
        int durationSeconds,
        ...
        CancellationToken ct = default)
    {
        if (!File.Exists(filePath))
            return new PublishClientResult { Success = false, Error = $"File not found: {filePath}" };
        ...
        var http = CreateClient();
        var publishResponse = await http.PostAsJsonAsync($"{_baseUrl}/podcast/publish", metadata, ct);

        if (!publishResponse.IsSuccessStatusCode)
        {
            var err = await publishResponse.Content.ReadAsStringAsync(ct);
            return new PublishClientResult { Success = false, Error = $"Publish failed: HTTP {(int)publishResponse.StatusCode} {err}" };
        }
        ...
        var upload = await http.PostAsync($"{_baseUrl}{uploadPath}", content, ct);
        if (!upload.IsSuccessStatusCode)
        {
            var err = await upload.Content.ReadAsStringAsync(ct);
            return new PublishClientResult { Success = false, Error = $"Upload failed: HTTP {(int)upload.StatusCode} {err}" };
        }

        return new PublishClientResult
        {
            Success = true,
            ...
            PublishTimestamp = metadata.PublishTimestamp
        };
    }
    ...
    {
        var http = CreateClient();
        return await http.GetFromJsonAsync<PodcastMetadata>($"{_baseUrl}/podcasts/{id}", ct);
    }

    private HttpClient CreateClient()
        => _httpFactory.CreateClient("windows-client-http");

    private static string BuildError(Exception ex)
    {
...
public class PublishClientResult : OperationResponse
{
